...
<body id="page-top">
  ...
  if (filter_var($result->path, FILTER_VALIDATE_URL) !== false) {
  $path = $result->path;


  } else {
  $path = "csv/".$result->path;
  ...
  if (($handle = fopen($path, "r")) !== FALSE) {
      while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
          ...
          for ($i=0; $i < $num; $i++) {
             $axis =  explode(";", $data[$i]);
             array_push($xaxis1,$axis[0]);
             array_push($yaxis1,$axis[1]);
             array_push($yaxis2,$axis[1]+$axis[2]);
             array_push($yaxis3,$axis[1]-$axis[2]);
          }
      }
      fclose($handle);
                    ...
                    <div class="col-lg-7 col-xl-8">
                        <div class="card shadow mb-4">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <div class="dropdown"><button class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-expanded="false" type="button" style="filter: blur(0px) brightness(81%) contrast(96%);">Seleziona CSV </button>
                                    <div class="dropdown-menu" role="menu">

                                      <?php
                                        while ($row = mysql_fetch_assoc($resultList)) {
                                      ?>
                                      <a class="dropdown-item" role="presentation" href="index.php?id=<?php echo $row['id']; ?>"><?php echo $row['id']; ?></a>
                                      <?php
                                       }
                                      ?>
